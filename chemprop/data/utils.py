...
def get_data(path: str,
    ...
    with open(path) as f:
        ...
        #print('data/utils.py print(target_columns)',target_columns); input()

        all_smiles, all_proteins, all_sps, all_targets, all_rows, all_features, all_weights = [], [], [], [], [] ,[], []
        ...
        for i, row in enumerate(tqdm(reader)):
            ...
            #print("data/utils.py targets :",targets); input()
        ...
        data = MoleculeDataset([
            ...
            ) for i, (smiles,sequences,sps, targets) in tqdm(enumerate(zip(all_smiles,all_proteins,all_sps,all_targets)),total=len(all_smiles))
        ])
...
def split_data(data: MoleculeDataset,
    ...
    if split_type == 'crossval':
        ...
        #print("split by random:?"); input()
        indices = list(range(len(data)))
        random.shuffle(indices)
        #print("indices:",indices[:20]);input()
...
def validate_dataset_type(data: MoleculeDataset, dataset_type: str) -> None:
    ...
    #print("data/utils.py : target_set ", target_set);input()
...
def get_vocabs(args : TrainArgs):
    SPS = ['ANKM', 'BETS', 'AEDL', 'ANKS', 'AEDM', 'ANTL', 'BETM', 'BNDL', 'CNTM', 'BEGM', 'BEKS', 'CNKL', 'ANKL', 'CEKS', 'CNKS', 'CNDS', 'AETM', 'BNGS', 'CEGS', 'BNKL', 'CNDM', 'BEGL', 'CNDL', 'BNGM', 'BNTS', 'ANTM', 'AEDS', 'ANDL', 'AEKM', 'BNTM', 'BEKM', 'ANGS', 'AEGM', 'AETL', 'BNKS', 'AEGS', 'CEDS', 'BNKM', 'AEKL', 'ANTS', 'CEDL', 'BEDS', 'CETS', 'BNTL', 'CEKM', 'ANDS', 'CNKM', 'BETL', 'BNGL', 'BNDS', 'CEGL', 'CNTS', 'BEGS', 'CNTL', 'CEKL', 'AETS', 'CETM', 'BNDM', 'CETL', 'BEDM', 'BEDL', 'AEGL', 'ANGM', 'CNGL', 'CEDM', 'CEGM', 'ANDM', 'BEKL', 'AEKS', 'ANGL', 'CNGS', 'CNGM']

    common_path = args.vocab_path # MUST CHAGNE DIRECTORY WHERE YOUR VOCAB FILES ARE

    # protein vocab #
    # vocab_path = common_path + 'protein_codes_uniprot.txt'
    # sub_csv = pd.read_csv(common_path + 'subword_units_map_uniprot.csv')
    vocab_path = common_path + 'protein_codes_uniprot_2000.txt'
    sub_csv = pd.read_csv(common_path + 'subword_units_map_uniprot_2000.csv')
    ...
    # drug vocab #
    # vocab_path = common_path + 'drug_codes_chembl.txt'
    # sub_csv = pd.read_csv(common_path +'subword_units_map_chembl.csv')
    vocab_path = common_path + 'drug_codes_chembl_freq_1500.txt'
    sub_csv = pd.read_csv(common_path +'subword_units_map_chembl_freq_1500.csv')

    bpe_codes_drug = codecs.open(vocab_path)
    dbpe = BPE(bpe_codes_drug, merges=-1, separator='')

    idx2word_d = sub_csv['index'].values
    words2idx_d = dict(zip(idx2word_d, range(2, len(idx2word_d)+2)))
    #############

    # SPS vocab #
    words2idx_s = {SPS[i]: i+2 for i in range(len(SPS))}
    #############
    
    # Adding Pad/Unknown Tokens
    words2idx_p['<pad>'] = 0
    words2idx_s['<pad>'] = 0
    words2idx_d['<pad>'] = 0
    words2idx_p['<unk>'] = 1
    ...
    return OrderedDict(words2idx_p), OrderedDict(words2idx_s), OrderedDict(words2idx_d), pbpe, dbpe

def protein2emb_encoder(x, words2idx_p, pbpe, args : TrainArgs):
    # print("for predicts , words2idx_p : ", words2idx_p); input()
    # print("pbpe : ", pbpe); input()
    t1 = pbpe.process_line(x).split()  # split
    i1 = np.asarray([words2idx_p[i] if i in words2idx_p else 1 for i in t1])  # index

    l = len(i1)

    if l < args.sequence_length:
        i = np.pad(i1, (0, args.sequence_length - l), 'constant', constant_values = 0)
    else:
        i = i1[:args.sequence_length]

    return i

def drug2emb_encoder(x, words2idx_d, dbpe, args : TrainArgs):
    t1 = dbpe.process_line(x).split()  # split
    i1 = np.asarray([words2idx_d[i] if i in words2idx_d else 1 for i in t1])  # index

    l = len(i1)

    if l < args.smiles_length:
        i = np.pad(i1, (0, args.smiles_length - l), 'constant', constant_values = 0)
    else:
        i = i1[:args.smiles_length]

    return i

def SPS2emb_encoder(x, words2idx_s, args: TrainArgs):
    #print(words2idx_s)

    #print("SPS origin sequence :", x)
    #input()

    t1 = x.split(',')
    #print("SPS after split :", t1)
    #input()
    i1 = np.asarray([words2idx_s[i] if i in words2idx_s else 1 for i in t1])
    
    l = len(i1)

    if l < args.sps_length:
        i = np.pad(i1, (0, args.sps_length - l), 'constant', constant_values = 0)
        # input_mask = ([1] * l) + ([0] * (config['max_SPS_seq'] - l))
    else:
        i = i1[:args.sps_length]
        # input_mask = [1] * config['max_SPS_seq']
    #print("embedded sps: ",i)
    #input()
    return i
