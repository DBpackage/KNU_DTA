...
def initialize_weights(model: nn.Module) -> None:
    ...
    :param model: An PyTorch model.
    """
    #print(model.named_parameters()); input()
    #print(model.parameters()); input()
    
    #for name, parameter in model.named_parameters():
    #    print(name)

    #input("asdfkjhasdfulasdhflkduqahflidf:")
    

    #print(sum(p.numel() for p in model.parameters()), sum(p.numel() for n, p in model.named_parameters())); input()
    #cnt = 0
    for name, param in model.named_parameters():
        if 'LN.weight' in name or 'norm' in name:
            #cnt += 1
            continue
        if param.dim() == 1:
