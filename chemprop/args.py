...
Metric = Literal['auc', 'prc-auc', 'rmse', 'mae', 'mse', 'r2', 'ci', 'accuracy', 'cross_entropy', 'binary_cross_entropy', 'pearson', 'spearman']
...
class TrainArgs(CommonArgs):
    ...
    # SPS CNN arguments
    sps_hidden_size : int = 128 # sps Embedding size for 1D CNN
    sps_1d_out      : int = 64  # sps 1D CNN len out_channels
    kernel_size     : int = 7   # sps 1D CNN kernel size
    sps_1dcnn_numlayers : int = 3 # sps 1D CNN number of layer iteration
    
    # model ablation argumentsi
    prot_cnn            : bool = False
    sps_cnn             : bool = False
    D_MPNN              : bool = False
    ECFP                : bool = False
    ...
    def process_args(self) -> None:
        ...
        for metric in self.metrics:
            if not ((self.dataset_type == 'classification' and metric in ['auc', 'prc-auc', 'accuracy', 'binary_cross_entropy']) or
                    (self.dataset_type == 'regression' and metric in ['rmse', 'mae', 'mse', 'r2', 'ci', 'pearson', 'spearman']) or
